...
//------------------------------------------------------------------------------
//
//				Time-stamp: "2021-03-15 00:28:38 shigeo"
//
//==============================================================================
...
void CSVIO::csvline( string & line, ublas::vector< double, std::vector< double > > & seq )
{
    std::vector< string > strvec = CSVIO::split( line, ',' );
...
void CSVIO::sheet( const char * filename, ublas::matrix< double > & table )
{
    table.clear();
    ...
    std::vector< ublas::vector< double > > dseq;
    string line;
    int nVars = 0;
    while ( getline( ifs, line ) ) {
	ublas::vector< double, std::vector< double > > seq;
	csvline( line, seq );
	nVars = std::max( nVars, ( int )seq.size() );
	dseq.push_back( seq );
    }

    table.resize( dseq.size(), ( unsigned int )nVars );
    for ( unsigned int k = 0; k < table.size1(); ++k )
	ublas::row( table, k ) = dseq[ k ];
	...
	for ( unsigned int j = 0; j < data[ i ].size(); ++j ) {
	    cerr << setw( 4 ) << setfill( ' ' ) << data[ i ][ j ] << ends;
	}
	cerr << endl;
    } 
#endif	// DEBUG
